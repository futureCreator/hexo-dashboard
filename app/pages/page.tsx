...
import { readPages, hexoPathValid } from "@/lib/hexo";

export const dynamic = "force-dynamic";

async function getPagesData() {
  const { hexoPath } = loadSettings();
  if (!hexoPath) return { configured: false, pages: [], hexoPath: "" };
  if (!hexoPathValid(hexoPath)) return { configured: true, valid: false, pages: [], hexoPath };
  const pages = readPages(hexoPath);
  return { configured: true, valid: true, pages, hexoPath };
}
      ...
      <div className="px-8 py-10 max-w-6xl">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="text-[var(--muted-foreground)] text-sm">
              Manage standalone pages like About, Contact, and more.
            </p>
          </div>
        </div>
        ...
        {!configured && (
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-12 text-center">
            ...
            <p className="text-sm text-[var(--muted-foreground)] mb-6 max-w-sm mx-auto">
              Set your local Hexo blog directory path to start managing your pages.
            </p>
            <Link
              href="/settings"
        ...
        {configured && !valid && (
          <div className="rounded-2xl border border-red-100 bg-red-50 p-12 text-center">
            ...
            <h2 className="text-xl font-semibold text-red-800 mb-2">Invalid Hexo path</h2>
            <p className="text-sm text-red-600 mb-6 max-w-sm mx-auto">
              The configured path no longer exists or is missing a{" "}
              <code className="font-mono text-xs bg-red-100 px-1.5 py-0.5 rounded">source/_posts</code>{" "}
